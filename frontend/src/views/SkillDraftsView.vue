...
import { deleteWorkflowDraft, fetchWorkflows, updateWorkflowDraft, type Workflow } from '../api/workflows'
...
const selectedSampleTasks = computed(() => {
  const raw = selectedDraft.value?.sample_tasks as unknown
  if (!raw) return []
...
function sourceLabel(draft: Workflow) {
  ...
  if (source.includes('existing')) return '已存在自动'
  return '来源未标注'
}
...
<template>
  <section class="page-view" v-loading="loading">
    <div class="legend">
      <el-tag type="warning">手动进化</el-tag>
      <span>从“已存在 Skills”点击进化创建，可编辑、删除。</span>
      <el-tag type="primary">进化管道推荐</el-tag>
      <span>由扫描、聚类、生成草稿流程自动产生。</span>
      <el-tag type="success">大模型复核</el-tag>
      <span>经过 LLM 重新分析或优化的推荐。</span>
    </div>

    <div v-if="drafts.length > 0" class="draft-layout">
      <div class="draft-list">
        ...
        >
          ...
          <div class="draft-meta">
            ...
            <span>评分 {{ draft.skill_score }}</span>
          </div>
        </article>
      </div>
